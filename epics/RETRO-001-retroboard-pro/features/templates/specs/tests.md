...
## 1. Integration Tests — Phase 1 (GET-only)

> **Note:** Phase 1 templates scope is limited to 6 system templates with read-only access (2 endpoints: list and detail). Custom template CRUD is deferred to Phase 5.

### 1.1 GET /api/v1/templates — List Templates

**File:** `tests/integration/templates/list-templates.test.ts`

| # | Test Case | HTTP | Expected |
|---|-----------|------|----------|
| 2.1.1 | List templates without team_id filter | 200 | All 6 system templates returned |
| 2.1.2 | List templates with team_id filter | 200 | 6 system templates (no custom templates in Phase 1) |
| 2.1.3 | List templates with include_system=false | 200 | Empty list (no custom templates in Phase 1) |
| 2.1.4 | List templates as unauthenticated user | 401 | UNAUTHORIZED |
| 2.1.5 | System templates listed in correct order | 200 | Correct ordering by position/name |
| 2.1.6 | Each template includes column_count | 200 | column_count matches actual columns |
| 2.1.7 | Custom templates from other teams not visible | 200 | Only system templates returned for non-member team_id |
| 2.1.8 | User in multiple teams sees correct templates | 200 | System templates always included |

### 1.2 GET /api/v1/templates/:id — Get Template Detail
...
| 2.2.4 | Get custom template from another team (IDOR check) | 404 | TEMPLATE_NOT_FOUND — verifies team membership check prevents cross-team access |
...
## 4. Test Coverage Requirements — Phase 1

| Area | Minimum Coverage |
|------|-----------------|
| Template list/detail handlers | 95% |
...
async function setupTemplates() {
  ...
  // Create a custom template for testing (via direct DB insert for Phase 1)
...
## Phase 5 — Deferred: Custom Template CRUD Tests

> The following tests are deferred from Phase 1 to Phase 5 when custom template CRUD is implemented.
...
| 1.1.16 | Template column with shorthand hex `"#fff"` | Validation error: must be 6-digit hex (`^#[0-9a-fA-F]{6}$`) |
| 1.1.17 | Template column with negative position `-1` | Validation error: position must be >= 0 |
| 1.1.18 | Template name with only whitespace `"   "` | Validation error: name required (after trim) |
| 1.1.19 | Template column name with only whitespace `"   "` | Validation error: column name required (after trim) |

#### Template Column Update Logic
...
#### POST /api/v1/teams/:teamId/templates — Create Custom Template
...
| 2.3.18 | Column with shorthand hex `"#fff"` rejected | 422 | VALIDATION_ERROR — must be 6-digit hex |
| 2.3.19 | Column with negative position `-1` rejected | 422 | VALIDATION_ERROR — position must be >= 0 |
| 2.3.20 | Concurrent creation with same name in same team | 201/409 | One succeeds, other gets TEMPLATE_NAME_TAKEN (DB UNIQUE) |

#### PUT /api/v1/teams/:teamId/templates/:id — Update Custom Template
...
#### DELETE /api/v1/teams/:teamId/templates/:id — Delete Custom Template
